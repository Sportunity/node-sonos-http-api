...
// Create backup preset to restore this player
  ...
  const currentPlayerZone = player.system.zones.find(zone => zone.members.findIndex(member => member.roomName === player.roomName) >= 0)

  // console.log("currentPlayerZone", currentPlayerZone)
  console.log("currentPlayerZone.coordinator.avTransportUri START", currentPlayerZone.coordinator.avTransportUri)
  let avTransportUri = currentPlayerZone.coordinator.avTransportUri;
  let trackNo = currentPlayerZone.coordinator.state.trackNo; 
  let elapsedTime = currentPlayerZone.coordinator.state.elapsedTime; 

  let groupToRejoin;

  const backupPreset = {
    players: 
      [{roomName: currentPlayerZone.coordinator.roomName, volume: currentPlayerZone.coordinator.volume}]
      .concat(
        currentPlayerZone.members.filter(member => member.roomName !== currentPlayerZone.coordinator.roomName).map(member => ({
  ...
  if (player.coordinator.uuid == player.uuid) {
    ...
    backupPreset.uri = avTransportUri; // `x-rincon:${player.coordinator.uuid}`;
    backupPreset.trackNo = trackNo ;
    console.log("---- elapsedTime", elapsedTime)
    console.log("---- duration", duration)
    backupPreset.elapsedTime = Math.round(elapsedTime + (duration + 2000) / 1000) ;
  }
...
// Use the preset action to play the tts file
  ...
  const prepareBackupPreset = (timer = 0) => {
...
console.log("found group zone = ", JSON.stringify(zone))
console.log("zone.coordinator.avTransportUri", zone.coordinator.avTransportUri);
console.log("currentPlayerZone.coordinator.avTransportUri", currentPlayerZone.coordinator.avTransportUri)

console.log("---- elapsedTime", elapsedTime)
    console.log("---- duration", duration)
    console.log("---- timer", Date.now() - timer)
relevantBackupPreset.uri = avTransportUri;
relevantBackupPreset.trackNo = trackNo ;
relevantBackupPreset.elapsedTime = Math.round(elapsedTime + (duration + 2000 + Date.now() - timer) / 1000) ;
      }
    }
  ...
  return system.applyPreset(ttsPreset)
    ...
    .then(() => prepareBackupPreset(timer))
    .catch((err) => {
      logger.error(err);
